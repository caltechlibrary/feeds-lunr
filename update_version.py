...
def update_version_text(version_text, current_version, next_version):
    with open(version_text, mode = "r", encoding = "utf-8") as f:
        src = f.read()
    txt = src.replace(f"Version = `v{current_version}`", f"Version = `v{next_version}`")
    with open(version_text, mode = "w", encoding = "utf-8") as f:
        f.write(txt)
    print(f"updated {version_text} Version from v{current_version} to v{next_version}")
    return True 
...
def main(args):
    ...
    if ("--yes" in args) or ("-yes" in args) or ("-y" in args):
        ...
        update_version_text(VERSION_TEXT, current_version, next_version)
    else:
        print("current version:", current_version)
